...
function saveBaseOptions(callback) {
	...
	appSettings.reloadBeforeSeconds = document.getElementById('reloadBeforeSeconds').value;
...
function restoreOptions() {
	...
	document.getElementById('reloadBeforeSeconds').value = appSettings.reloadBeforeSeconds || 3;
...
function generateAdvancedSettingsHtml(tab, saved) {
	...
	if (saved) {
		...
		secondsChunk =
			'<p><label for="seconds">Seconds:</label> <input type="text" name="seconds" value="' +
			tab.seconds +
			'" style="width:30px;">';
		if (tab.reload) {
			reloadChunk =
